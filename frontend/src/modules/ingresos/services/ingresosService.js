...
import { productosMock } from '../../productos/mocks/productosMock';
...
const normalizarProductos = () => {
  ...
  const base = [...productosMock, ...productosAdicionales];
  ...
  const unicos = base.reduce((acc, prod) => {
    ...
    const estimado =
      prod.duracion_estimada_dias || prod.vida_util_dias || estimacionesPorCategoria[keyCategoria] || 30;
    acc.push({
      id: prod.id,
      nombre: prod.nombre,
